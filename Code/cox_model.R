...
cox_model <- function(my_seed        = 123,
                             my_alpha       = 1,
                             cox_df         = NULL,
                             gene_num       = 1800,
                             max_it         = 100000,
                             ...
                             tumor_m        = FALSE,
                             n_folds        = 10,
                             remove_stage   = c("tumor.stage1", "tumor.stage2",
                                                "tumor.stage3", "tumor.stage4"),
                             remove_n_stage = c("ajcc.n0", "ajcc.n1", "ajcc.n2",
                                                "ajcc.n3"),
                             save_coefs    = FALSE,
                             calc_auc      = FALSE,
  ...
  my_y <- Surv(time = cox_df$time,
   event = cox_df$vital.status)
  
  
  
  #The 10-fold cross-validation fit
  cv_fit <- cv.glmnet(x = my_x, y = my_y, nfolds = n_folds, type.measure = "C",
  ...
  if (calc_auc == TRUE) {
    model_perf <- assess.glmnet(cv_fit, newy = my_y,
     family = "cox", s = "lambda.min")
    print(paste0("The model performance on the entire data set is ",
     model_perf$C))
