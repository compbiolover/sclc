...
scl_patient <- scl_patient %>%
  ...
  rename_all(tolower) %>%
  # Uncomment this line for the non-progress free analysis
...
# KM Plot. Unbalanced dataset. Will need to work on this more as we currently have problems
```{r risk seperation}
risk_genes <- data.frame(gene = c("tfrc", "fam83f", "samd12", "grin2b", "dlk1", "gng13"), coef = c(-0.75737985824054, -0.0706471525401676, -0.16554139965723, 0.305795111401098, -0.000374799303479439, -0.027261204056091))

write.csv(risk_genes, "Outputs/mirna_mad_sde_coefs.csv")

km_plot <- risk_score_calculator(
  my.file = "Outputs/mirna_mad_sde_coefs.csv",
  tumor.data = FALSE,
  n.data = FALSE,
  set.ci = TRUE,
  cox.df = scl_common,
...
set.seed(1)
# Assume your original dataset is named `mydata` and it has a column named `vital.status`
...
# Random Genes on 1000 simulated datasets
```{r random genes 1000}
scl_common <- scl_common %>%
  select(
    -patient.id, -sex, -smoker, -vital.status, -time, -n.stage, -m.stage,
...
for (g in sampled_genes) {
  current_genes <- g
  current_genes <- colnames(current_genes)
  for (d in 1:length(bootstrap_datasets)) {
    current_dataset <- bootstrap_datasets[[d]]
    current_cox <- cox_model_fitter(my_seed = 1, my_alpha = 0,
...
# calculate the mean performance across all folds
mean_performance <- mean(cv_performance) # 0.X with the 7 active predictors from random genes
```
...
plot_df <- data.frame(metric = c("M", "S", "miR", "CS", "RG", "MS", "miRM", "miRS", "miRMS"), cindex = c(0.6212627, 0.6746058, 0.6920965, "0.X", "0.X", "0.6958149", "0.6907635", "0.6907635", "0.7204482"))
...
# calculate the mean performance across all folds
mean_performance <- mean(cv_performance) # 0.7460983 with the 6 active perdictors from mad + sde + miRNA
```
...
# Attempt to use permutation test to seperate high and low risk group
```{r now using permutation test to see if we have high and low risk separation}
# Assume your 6-gene predictor is stored in a vector called 'risk_scores'
# Assume your vital status information (0 for alive, 1 for dead) is stored in a vector called 'vital_status'
# Assume you want to perform 1000 permutations

risk_scores <- c("tfrc", "fam83f", "samd12", "grin2b", "dlk1", "gng13")

set.seed(1) # for reproducibility

# Define a function to calculate the difference in median survival time between the high and low-risk groups
med_diff <- function(risk, vital) {
  high_risk <- risk >= median(risk)
  low_risk <- risk < median(risk)
  med_diff <- median(vital[high_risk]) - median(vital[low_risk])
  return(med_diff)
}

# Calculate the observed median difference in survival time between the high and low-risk groups
observed_med_diff <- med_diff(risk_scores, scl_common$vital.status)

# Perform the permutation test
perm_med_diffs <- replicate(1000, {
  perm_vital_status <- sample(vital_status)
  med_diff(risk_scores, perm_vital_status)
})

# Calculate the p-value
p_value <- mean(abs(perm_med_diffs) >= abs(observed_med_diff))

# Plot the distribution of median differences under permutation
hist(perm_med_diffs, main = "Distribution of Median Differences under Permutation", xlab = "Median Difference")

# Add a vertical line indicating the observed median difference
abline(v = observed_med_diff, col = "red")

# Add text indicating the p-value
text(-5, 80, paste0("p-value: ", round(p_value, 3)))
```
