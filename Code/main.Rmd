...
scl_common <- scl_common %>%
  select(patient.id, sex, smoker, vital.status, time, time2, n.stage, m.stage,
         uicc.tumor.stage, chemotherapy, neoadjuvant.chemotherapy,
...
# Attempt to find other datasets
...
cell_lines <- cell_lines %>%
  select(-TranscriptClusterID, -UNIT_ID, -GeneName, -GeneAccession, -EntrezID, -Chromosome, -Cytoband, -Start, -Stop, -Strand, -CrossHybridization, -ProbesetType)

# Generating the survival distribution from our existing dataset
surv_model <- survfit(Surv(time, vital.status) ~ 1, data = scl_common)
surv_distribution <- summary(surv_model)$surv

# Now using the survival distribution to add vital.status and survival times to the cell line data
# Assuming you have a vector of survival times and statuses in the 'surv_times' and 'surv_statuses' objects

# Number of cell lines
num_cell_lines <- ncol(cell_lines) - 1


# Number of synthetic datasets to generate
num_datasets <- 1000

# Number of synthetic times and statuses to generate for each dataset
num_synthetic <- 1000


# Create an empty list to store the synthetic survival data for each dataset
synthetic_survival_data <- vector("list", length = num_datasets)


rownames(cell_lines) <- cell_lines$GeneSymbol
cell_lines_sub <- cell_lines %>% select(-GeneSymbol)

# Generate synthetic survival times and statuses for each dataset and each cell line
for (i in 1:num_datasets) {
  synthetic_times <- replicate(num_synthetic, sample(surv_times, size = num_cell_lines, replace = TRUE))
  synthetic_statuses <- replicate(num_synthetic, sample(surv_statuses, size = num_cell_lines, replace = TRUE))
  
  # Create a new dataframe with the synthetic survival data
  synthetic_survival_data[[i]] <- data.frame(
    cell_line = colnames(cell_lines_sub),  # Assuming your gene expression matrix has column names for cell lines
    time = synthetic_times,
    vital.status = synthetic_statuses
  )
}

# surv_times <- scl_common$time
# surv_statuses <- scl_common$vital.status
# 
# # Generate synthetic survival times and statuses for each cell line
# synthetic_times <- sample(surv_times, size = num_cell_lines, replace = TRUE)
# synthetic_statuses <- sample(surv_statuses, size = num_cell_lines, replace = TRUE)
# 
# # Create a new dataframe with the synthetic survival data
# cell_lines_sub <- cell_lines %>% select(-GeneSymbol)
# synthetic_survival_data <- data.frame(
#   cell_line = colnames(cell_lines_sub),  # Assuming your gene expression matrix has column names for cell lines
#   time = synthetic_times,
#   vital.status = synthetic_statuses
# )
