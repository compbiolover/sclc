...
# Set a seed based on the system time
...
cox_mod_test <- coxph(data = simulated_dataset, Surv(time, status) ~ tfrc + fam83f + samd12 + grin2b + dlk1 + gng13) # C-index of 0.69. With seed from above (1688405764) set it is 0.803 [se = 0.074]

# 5 fold CV because of small size
cox_preds <- data.frame(preds = c("tfrc", "fam83f", "samd12", "grin2b", "dlk1", "gng13"))
rownames(cox_preds) <- cox_preds$preds
...
# C-index for 5-fold CV is 0.863 with se = 0.06083
current_cox <- cox_model_fitter( 
  my_seed = 1688405764,
  my_alpha = 0,
  progress_free = FALSE,
...
cox_mod_test2 <- coxph(data = simulated_dataset2, Surv(time, status) ~ tfrc + fam83f + samd12 + dlk1 + gng13) # 0.629 for 7% survival rate, 0.756 for original dataset censoring rate (36%) [random seed: 1688405764]

simulated_dataset2_glmnet <- simulated_dataset2
colnames(simulated_dataset2_glmnet)[22282] <- "vital.status"

# 5 fold cv (0.7368 with seed 123, 0.6211 with seed 1685733101, 0.7733 with seed used by rest of simulated datasets:1688405764 )
...
# Now testing the signature.
cox_mod_test3 <- coxph(data = simulated_dataset3, Surv(time, status) ~ tfrc + fam83f + samd12 + dlk1 + grin2b + gng13) # 0.615 C-index on initial dataset censoring rate. 0.659 with random seed with the seed used for other datasets. 


colnames(simulated_dataset3)[51081] <- "vital.status"
...
# Bifurcating each gene in the gene signature of miRMS into high and low risk based on mean expression to see how expression impacts survival
