...
genes <- c("tfrc", "fam83f", "samd12", "grin2b", "dlk1", "gng13")
new_gene_dataset <- read.csv("Data/GSE149507_geo_expr.csv")
new_gene_dataset <- new_gene_dataset %>%
  filter(!is.na(Symbol))
rownames(new_gene_dataset) <- new_gene_dataset$Symbol
new_gene_dataset <- new_gene_dataset %>%
...
n <- nrow(new_gene_dataset)  # Number of individuals in the new dataset
...
simulated_dataset <- cbind(new_gene_dataset, time = simulated_times, status = censoring_status)
colnames(simulated_dataset) <- tolower(colnames(simulated_dataset))


# Now testing the signature
cox_mod_test <- coxph(data = simulated_dataset, Surv(time, status) ~ tfrc + fam83f + samd12 + grin2b + dlk1 + gng13) # C-index of 0.69
...
# current_cox <- cox_model_fitter( C-index of 0.7037 
#   my_seed = 1234,
#   my_alpha = 0,
#   progress_free = FALSE,
#   sim_testing = TRUE,
...
# Now loading the next validation dataset (https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE110853 [GEO: GSE110853])
# next_test_df <- read.csv("Data/GSE110853_TPM_matrix_GEO.csv")
# next_test_df <- next_test_df %>%
#   filter(row != 6020)
# next_test_df <- next_test_df %>%
#   filter(row != 11468)
# rownames(next_test_df) <- next_test_df$hgnc_symbol
# next_test_df <- next_test_df %>%
#   select(-row, -hgnc_symbol)
# 
# next_test_df <- as.data.frame(next_test_df)
# next_test_df <- t(next_test_df)
...
# Simulate survival times for the new gene expression dataset
n <- nrow(next_test_df)  # Number of individuals in the new dataset
simulated_times <- sample(time_points, size = n, replace = TRUE, prob = surv_prob)

# Optionally, simulate censoring based on a desired censoring rate
censoring_rate <- 0.361  # Desired censoring rate
censoring_status <- ifelse(runif(n) < censoring_rate, 0, 1)  # 0 represents censoring, 1 represents event
...
# Now testing the signature. grin2b isn't in this dataset so I can only test the other genes
cox_mod_test2 <- coxph(data = simulated_dataset2, Surv(time, status) ~ tfrc + fam83f + samd12 + dlk1 + gng13) # 0.629 for 7% survival rate, 0.744 for original dataset censoring rate (36%)

simulated_dataset2_glmnet <- simulated_dataset2
colnames(simulated_dataset2_glmnet)[22282] <- "vital.status"

# 5 fold cv (0.7368 with seed 123, 0.6211 with seed 1685733101)
...
# Now testing many random seeds to try and minimize bias of seed selection and get a fair view of
# performance

seeds <- numeric()

for (n in 1:1000){
  current_seed <- as.integer(runif(1, min = 1e6, max = 1e9))
  seeds[[n]] <- current_seed
}
...
c_indicies <- numeric()
for (n in 1:length(cox_outputs)){
  current_cox <- cox_outputs[[n]]
  c_index <- current_cox$CV$cvm[current_cox$CV$index[1]]
  c_indicies[[n]] <- c_index
}


# 3rd dataset processing
third_df <- read.csv("Data/GSE60052_79tumor.7normal.normalized.log2.data.Rda_finished.csv")
...
simulated_dataset3 <- simulated_dataset3 %>%
  select(!contains("hsa.mir."))

# Now testing the signature.
cox_mod_test3 <- coxph(data = simulated_dataset3, Surv(time, status) ~ tfrc + fam83f + samd12 + dlk1 + grin2b + gng13) # 0.615 C-index on initial dataset censoring rate 

# Trying bootstrappping
# Assuming you have a dataset called "simulated_dataset3" with gene expression information
# Set the number of bootstraps
num_bootstraps <- 1000
mod_results <- list()

# Create empty vectors to store the bootstrapped survival times and censoring status
bootstrapped_times <- vector("list", length = num_bootstraps)
bootstrapped_status <- vector("list", length = num_bootstraps)

# Perform bootstrapping
set.seed(1685733101)  # Set a seed for reproducibility
for (i in 1:num_bootstraps) {
  # Generate a bootstrap sample by sampling with replacement from the dataset
  bootstrap_sample <- simulated_dataset3 %>%
    sample_n(size = nrow(simulated_dataset3), replace = TRUE)
  
  # Extract the survival times and censoring status from the bootstrap sample
  bootstrapped_times[[i]] <- bootstrap_sample$time
  bootstrapped_status[[i]] <- bootstrap_sample$status
}

# Perform the analysis on each bootstrap sample
for (i in 1:num_bootstraps) {
  # Fit the Cox proportional hazards model on the bootstrap sample
  model <- coxph(Surv(bootstrapped_times[[i]], bootstrapped_status[[i]]) ~ tfrc + fam83f + samd12 + dlk1 + grin2b + gng13, data = simulated_dataset3)
  
  # Calculate the concordance index using the full dataset
  mod_sum <- summary(model)
  mod_results[[i]] <- mod_sum$concordance[1]
}
...
rm(simulated_datasets, simulated_gene_datasets)
```

```{r testing miRMS signature on cell lines}
# Renaming status column to vital.status for cox model code
# Function to rename a column in a data frame using dplyr
...
finished_datasets <- lapply(finished_datasets, rename_column, old_name = "status", new_name = "vital.status")

cox_outputs <- list()
counter <- 1
cox_preds <- data.frame(preds = c("tfrc", "fam83f", "samd12", "grin2b", "dlk1", "gng13"))
rownames(cox_preds) <- cox_preds$preds
for (d in 1:length(finished_datasets)) {
  current_dataset <- finished_datasets[[d]]
  current_cox <- cox_model_fitter(
    my_seed = 1,
    my_alpha = 0,
    sim_testing = TRUE,
...
# calculate the mean performance across all folds
mean_performance <- mean(cv_performance) # 0.7204482 with the 6 active perdictors from mad + sde + miRNA
median_performance <- median(cv_performance)
```
