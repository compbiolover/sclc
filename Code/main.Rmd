...
pacman::p_load(
  cowplot, coxed, data.table, devtools, ggplot2, glmnet, hoardeR, igraph, Matrix, monocle3, parallel, phateR,
...
plot_df <- data.frame(metric = c("M", "S", "miR", "CS", "MS", "miRM", "miRS", "miRMS"),
...
simdata <- sim.survdata(N = 72, T=4920, X=top_pred_df, num.data.frames = 1000, beta = c(-0.75737985824054, -0.0706471525401676, -0.16554139965723, 0.305795111401098, -0.000374799303479439, -0.027261204056091), censor = 0.362)
...
p1 <- ggsurvplot(fit = fit,
           ...
           ggtheme = theme(axis.line = element_line(color = "black", linewidth = 1.25, lineend = "round"),
                 axis.title = element_text(size = 18, face = "bold"),
                 plot.title = element_text(size = 24, face = "bold", hjust = 0.5),
