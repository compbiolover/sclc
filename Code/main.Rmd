...
pacman::p_load(
  cowplot, data.table, devtools, ggplot2, glmnet, hoardr, igraph, Matrix, monocle3, parallel, phateR,
  reticulate, Rmagic, styler, survminer, svglite, switchde, tidyverse, viridis
)
```
...
```{r sde metric}
invisible(sde_genes <- switchde_calculator(sc_denoised$result,
...
```{r mirna}
# Load the data
mirna_data <- read.csv("Data/mirna_data/hmdd-3-2-all-data.txt", sep = "\t")
...
mirna_num <- seq(10, 100, 5)
mirna_target_num <- seq(2, 8, 1)
for (m in mirna_num[1:19]) {
  for (t in mirna_target_num[1:7]){
    mirna_genes <- mirna_calculator(ts_org = "Human", ts_version = "8.0", max_mir_targets = 8, cancer_up = FALSE, cancer_type1 = "lung cancer", print_ts_targets = TRUE, max_mirnas = m, mirna_genes_mat_name = paste0("Outputs/decreased_mirna_targets/mirna_genes_mat_", m, "_mirnas_", t, "_targets_low.rds"), mirna_ranking_name = paste0("Outputs/decreased_mirna_targets/mirna_genes_", m, "_mirnas_", t, "_targets_low.rds"))
  }
}
```
