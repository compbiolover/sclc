...
cox_mod_test2 <- coxph(data = simulated_dataset2, Surv(time, status) ~ tfrc + fam83f + samd12 + dlk1 + gng13) # 0.629
...
# folds <- cut(seq(nrow(simulated_dataset2)), breaks = k, labels = FALSE)
# for (i in 1:k) {
#   # Create the training and testing sets
#   train_data <- simulated_dataset2[folds != i, ]
#   test_data <- simulated_dataset2[folds == i, ]
#   
#   # Fit the Cox proportional hazards model on the training data
#   model <- coxph(Surv(time, status) ~ tfrc + fam83f + samd12 + dlk1 + gng13, data = train_data)
#   
#   # Make predictions on the testing data
#   predictions <- predict(model, newdata = test_data, type = "expected")
#   
#   # Store the predictions in the corresponding fold index
#   cv_predictions[[i]] <- predictions
# }
# 
# # Combine the cross-validated predictions into a single object
# cv_predictions <- do.call(rbind, cv_predictions)
# 
# # Evaluate the performance of the cross-validated predictions
# # (e.g., calculate the concordance index or other relevant metrics)
# concordance <- survival::concordance(simulated_dataset2$time, cv_predictions, simulated_dataset2$status)
# print(paste("Concordance Index:", concordance))


# Set the number of folds for cross-validation
k <- 10

# Create an empty vector to store the concordance indices
concordance_indices <- vector("numeric", length = k)
...
for (i in 1:k) {
  ...
  # Make predictions on the testing data
  predicted_survival <- predict(model, newdata = test_data, type = "expected")
  
  # Calculate the concordance index
  concordance_indices[i] <- concordance(survival::Surv(test_data$time, test_data$status), predicted_survival)
}

# Calculate the average concordance index
average_concordance <- mean(concordance_indices)

print(paste("Average Concordance Index:", average_concordance))



# 3rd dataset processing
# third_df <- read.csv("Data/GSE60052_79tumor.7normal.normalized.log2.data.Rda.csv")
# third_df <- third_df %>%
#   filter(gene != "1-Mar") %>%
#   filter(gene != "2-Mar")
# third_df <- third_df %>%
#   t() %>%
#   as.data.frame()
# colnames(third_df) <- third_df["gene",]
# third_df <- third_df %>%
#   slice(2:87) %>%
#  filter(!grepl(".normal", rownames(.)))
# 
...
# Now testing the signature.
cox_mod_test3 <- coxph(data = simulated_dataset3, Surv(time, status) ~ tfrc + fam83f + samd12 + dlk1 + grin2b + gng13)
```
