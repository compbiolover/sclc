...
risk_score_calculator <- function(my.file = active_coefs.csv,
  ...
  if (tumor.data == FALSE & n.data == FALSE) {
    ...
    # Performing IPCW if needed
    if (perform_ipcw) {
      # Perform IPCW
      weights <- riskRegression::ipcw(Surv(time, vital.status) ~ risk,
                                      data = converted_df,
                                      method = "cox", 
                                      times = sort(unique(converted_df$time)),
                                      subject.times = converted_df$time,keep=c("fit"))
      
      # Fit the Cox model with IPCW
      cox_fit <- coxph(Surv(time, vital.status) ~ risk, data = converted_df, weights = weights)
      
      # Compute the survival estimates using the fitted Cox model
      km_fit <- survfit(cox_fit)
    } else {
