...
# Example usage with error handling for FPKM to TPM:
tryCatch({
    tpm_data <- fpkm_to_tpm(test_df)
...
# Checks distributions to see if log2 transformation is needed
check_distribution <- function(data, pseudocount = 1, sample_name = "sample", 
  ...
  p2 <- ggplot(data.frame(x = log_data), aes(x = x)) +
    geom_histogram(bins = 50) +
    ggtitle(paste0("Log2 Transformed - ", sample_name)) +
    theme_minimal()
...
#' Load SCLC clinical data
#' @param file_path Path to clinical data file
#' @param survival_type "overall" or "progression_free" 
#' @return Processed clinical data frame
load_clinical_data <- function(file_path, survival_type) {
 data <- read.csv(file_path, sep = "\t") %>%
   slice(5:121) %>% 
   rename_clinical_columns() %>%
   select_clinical_columns() %>%
   process_clinical_variables(survival_type)
 if(survival_type == "progression_free") {
   process_progression_free(data)}
 else {
   data 
 }
}
...
#' @param expr_data Expression data frame
#' @param common_patients Vector of common patient IDs
#' @return Processed expression data frame
process_expression_data <- function(expr_data, common_patients) {
 expr_data %>%
   select(all_of(common_patients)) %>%
   t() %>%
   as.data.frame() %>%
   `colnames<-`(expr_data$Hugo_Symbol) %>%
...
#' @param clinical_data Processed clinical data
#' @return Merged data frame
merge_expression_clinical <- function(expr_data, clinical_data) {
 clinical_cols <- c("sex", "smoker", "vital_status", "time", "patient_id",
                   "n_stage", "m_stage", "uicc_tumor_stage", "chemotherapy", 
                   "neoadjuvant_chemotherapy", "acquisition_method", "age",
                   "radiation")
 
 left_join(expr_data, clinical_data[, clinical_cols], by = "patient_id") %>%
   select(patient_id, age, sex, smoker, vital_status, time, n_stage, m_stage,
          uicc_tumor_stage, chemotherapy, neoadjuvant_chemotherapy,
          radiation, acquisition_method, everything())
}
...
rename_clinical_columns <- function(data) {
 rename(data,
   patient_id = X_Patient_Identifier,
   vital_status = Overall_Survival_Status, 
   survival_months = Overall_Survival_Months,
   progression_free_months = Progress_Free_Survival_Months,
   acquisition_method = First_Pathologic_Diagnosis_Biospecimen_Acquisition_Method_Type,
   age = Diagnosis_Age,
...
select_clinical_columns <- function(data) {
 select(data, patient_id, sex, vital_status, survival_months, 
        progression_free_months, smoker, chemotherapy, neoadjuvant_chemotherapy,
        n_stage, m_stage, uicc_tumor_stage, acquisition_method, age, radiation)
}
